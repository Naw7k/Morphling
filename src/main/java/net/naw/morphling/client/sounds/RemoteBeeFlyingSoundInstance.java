...
public class RemoteBeeFlyingSoundInstance extends AbstractTickableSoundInstance {

    private final Player remotePlayer;
    private final RemoteMorphState.PlayerMorphData data;
    private boolean hasSwitched = false;

    public RemoteBeeFlyingSoundInstance(Player remotePlayer, RemoteMorphState.PlayerMorphData data) {
        super(SoundEvents.BEE_LOOP, SoundSource.PLAYERS, SoundInstance.createUnseededRandom());
    ...
    public void tick() {
        ...
        if (data.beeAngry && !hasSwitched) {
            ...
            net.minecraft.client.Minecraft.getInstance().getSoundManager().queueTickingSound(
                    new RemoteBeeAggressiveSoundInstance(remotePlayer, data)
            );
            this.stop();
            return;
        }
        ...
        if (speed >= 0.01F) {
            ...
            this.volume = Mth.lerp(Mth.clamp(speed, 0.0F, 0.5F), 0.0F, 0.8F);
        } else {
            this.pitch = 0.7F;
            this.volume = 0.05F;
        }
